...
class DataChunk:
    ...
    def __init__(self, MODEL_NAME, case = 0, chunk_size=500):
    ...
    # chunking된 데이터를 post processing하는 함수 (chunk size로 나누는데, 이 때 e5 (case 0)의 경우에는 prefix를 추가해야함.)
    def chunk_post_processor(self, encoded_texts, titles, texts):
        ...
        # orig_text : 원본 text, orig_title : 원본 title, chunk_list : chunking된 text (encoding 되어 있음.)
        return  orig_text, orig_title, chunk_list
    
    # 위의 함수들을 활용해서 chunk를 진행
...
def save_orig_passage(
    MODEL_NAME, input_path="Extractor", passage_path="processed_passages", chunk_size=500
):
...
# 나중에 title과 matching하여 indexing하기 위해 필요한 함수 (processed_passages_title에 저장됨)
