...
from kortrieval_for_github.evaluate.korQuAD.utilsval_for_github.evaluate.korQuAD.utils import get_passage_file
import ast
...
class ValidationDataset(torch.utils.data.Dataset):
    def __init__(self, data: List[Tuple]):
        self.data = data
        self.data['query'] = self.data['query'].apply(ast.literal_eval)
        self.data['answer'] = self.data['answer'].apply(ast.literal_eval)
...
class KorQuadDataset:
    def __init__(self, korquad_path: str, title_passage_map_path="./title_passage_map.p", MODEL_TYPE = None, MODEL_NAME = None, chunk_size = 0):
    ...
    def load(self):
        ...
        self.korquad_processed_path = (
            f"{self.korquad_path.split('.json')[0]}_{self.MODEL_TYPE}_processed_{self.chunk_size}.p"
        )
        print(self.korquad_processed_path)
        if os.path.exists(self.korquad_processed_path):
            ...
            if self.MODEL_TYPE == 'e5':
                self.tokenized_tuples = [
                    (self.tokenizer.encode('query : '+ q), d, self.tokenizer.encode(p), answer)
                    for q, d, p, answer in tqdm(self.data_tuples, desc="tokenize")
                ]
            
            else :
                self.tokenized_tuples = [
                    (self.tokenizer.encode(q), d, self.tokenizer.encode(p), answer)
                    for q, d, p, answer in tqdm(self.data_tuples, desc="tokenize")
                ]
...
if __name__ == "__main__":
    ds = KorQuadDataset(korquad_path="./KorQuAD_v1.0_train.json")
